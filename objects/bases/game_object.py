...
class GameObject:
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, t in self.__class__.__annotations__.items():
            if not any([
                    hasattr(self, k),
                    isinstance(t, typing._UnionGenericAlias) and any(
                            isinstance(arg, typing._SpecialForm) and arg._name in ["Any", "NoneType"] for arg in
                            t.__args__),
                    isinstance(t, typing._SpecialForm) and t._name == "NoneType",
                    isinstance(getattr(self.__class__, k), BaseState) and (
                            getattr(self.__class__, k).default is not None or getattr(self.__class__,
                                                                                      k).default_factory is not None),
            ]):
                required_keys.add(k)
